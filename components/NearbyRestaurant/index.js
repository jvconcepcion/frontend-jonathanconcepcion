...
const RestaurantCards = ({ 
  ...
  const addOrder = () => {
    ...
    if(quantityRef.current.value !== "") {
      setOrderCount(quantityRef.current.value)
      setShowPopover(!showPopover);
      let final = {groceryId: id,
        userId: loginState.userId,
        name,
        quantity: parseInt(quantityRef.current.value),
        price: parseInt(price) * quantityRef.current.value
        }
      handleAddOrder(final)
    } else {
      setOrderCount(0)
    }
      ...
      <div className={styles.cardContainer_imgWrapper}>
        ...
        {JSON.parse(getCookie(LOGIN_API_PATH))?.permissionLevel === 1 && (
          <div className={styles.editAndDelete}>
            <div onClick={() => handleModal('Edit Product', true, { id, category, name, price }, 'Grocery')} className={styles.edit}>
        ...
        <div className={styles.nameAndAction}>
          ...
          <div ref={popOverRef}>
            ...
            <PopoverComponent 
              ...
              containerPadding={20}
              closeFunction={() =>setShowPopover(!showPopover)}
            >
              <div className={styles.addWrapper}>
                <input 
                  ref={quantityRef}
                  className={styles.addWrapper_quantity} 
                  type='number' 
                  placeholder='Quantity (e.g: 10)'
                />
                <button 
                  className={styles.addWrapper_addBtn}
                  onClick={addOrder}
                >Add</button>
              </div>
            </PopoverComponent>
  ...
  return (
    <div className={styles.nearbyRestaurantsContainer}>
      <div className={styles.headingWrapper}>
        ...
        <a onClick={() => handleModal('Add Product', true, '', 'Grocery')} className={styles.addProduct}>
